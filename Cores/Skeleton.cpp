...
#include "Skeleton.h"
#include <Windows.h>
#include "General/MurmurHash3.h"


Skeleton *me;

// Helper functions
bool Skeleton::FileExists(const char *path)
{
	DWORD dwAttrib = GetFileAttributes(path);
	return (dwAttrib != INVALID_FILE_ATTRIBUTES && !(dwAttrib & FILE_ATTRIBUTE_DIRECTORY));
}

uint32 Skeleton::getHash(HANDLE file)
{
	uint32 hash = 42;
	DWORD byteNo;
...
bool Skeleton::IsNewVersion(const char *pathLeft, const char *pathRight)
{
	...
	{
		DbgPrint("Error opening the files");
...
void Skeleton::DbgExport(const char* format, ...)
{
#ifdef _DEBUG
	//Reset the buffer
